...
#pragma omp parallel
    {
      ...
      oneapi::mkl::dft::compute_forward(
          *fft_yz_plans[i], dr::shp::__detail::local(atile).data()).wait();

#pragma omp barrier

        std::vector<sycl::event> events;
        for (int j_ = 0; j_ < nprocs; j_++) {
          int j = (j_ + i) % nprocs;
        ...
        oneapi::mkl::dft::compute_forward(
            *fft_x_plans[i], dr::shp::__detail::local(btile).data()).wait();
    }
  }
...
#pragma omp parallel 
    {
      ...
      oneapi::mkl::dft::compute_backward(
          *fft_x_plans[i], dr::shp::__detail::local(atile).data()).wait();
...
#pragma omp barrier
      ...
      auto &&btile = o_mat.tile({i, 0});
      oneapi::mkl::dft::compute_backward(
            *fft_yz_plans[i], dr::shp::__detail::local(btile).data()).wait();
    }
  }
};
